...
type UseCategoryImagesReturn = {
  ...
  saveToServer: (options?: {
    ...
    mergeLocal?: boolean;
  }) => Promise<{ ok: boolean; error?: string }>; // saves current state to GitHub via server API
...
export default function useCategoryImages(): UseCategoryImagesReturn {
  ...
  const updateImage = useCallback(
    (category: string, id: string, changes: Partial<CategoryImage>) => {
      setState((prev) => {
        const next = { ...(prev || {}) };
  ...
  const saveToServer = useCallback(
    async ({
      commitMessage,
      mergeLocal,
    }: { commitMessage?: string; mergeLocal?: boolean } = {}) => {

              ...
              <div key={img.id} style={{ display: "flex", gap: 8, alignItems: "center" }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ marginTop: 6 }}>
                    <button
                      onClick={() => setFeatured(category, img.id)}
                      style={{ marginRight: 8 }}
                    >
        ...
        <div>
          ...
          <div style={{ display: "grid", gap: 8 }}>
            ...
            <div style={{ marginTop: 12, fontSize: 13, color: "#666" }}>
              Tip: place images under <code>public/images/categories/{"{category}"}</code> so the
              placeholder structure matches the defaults.
            </div>
            ...
            <div style={{ marginTop: 12 }}>
              <button
                onClick={async () => {
                  const ok = await saveToServer({
                    commitMessage: `Admin: update category images (${category})`,
                    mergeLocal: true,
                  });
              ...
              {!serverAvailable && (
                <div style={{ color: "red", marginTop: 8 }}>
                  Server persistence unavailable (GITHUB_TOKEN not set).
                </div>
              )}
            </div>
          </div>
